...
-- Run this in the SQL editor while authenticated as your app user — otherwise
-- auth.uid() will be null and the inserts will fail the not-null user_id
-- constraint. Safe to re-run: duplicates are skipped via the (user_id, name)
-- unique constraint. Feel free to edit these lists to fit your own trading
-- rules before running — they're just a reasonable starting point.
